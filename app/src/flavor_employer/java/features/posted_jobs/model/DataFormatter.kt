...
class DataFormatter : KoinComponent {
     ...
     fun getJobsFromSnapshot(documents: QuerySnapshot): ArrayList<PostedJob> {
         ...
         for (document in documents) {
             ...
             jobs.add(PostedJob(
                     ...
                     applicants = getApplicantsStr(job.applicants),
                     status = JobUtil.KEY_JOB_STATUS_WAITING_TALENTS //todo de pus caoamne
             ))
         }
        ...
        return if (neededTime.get(Calendar.YEAR) == nowTime.get(Calendar.YEAR)) {
            ...
            if (neededTime.get(Calendar.MONTH) == nowTime.get(Calendar.MONTH)) {
                ...
                if (neededTime.get(Calendar.DATE) - nowTime.get(Calendar.DATE) == 1) {
                    ...
                    resUtil.getStringRes(R.string.txt_tomorrow) + " " + DateFormat.format("HH:mm", neededTime)

                } else if (nowTime.get(Calendar.DATE) == neededTime.get(Calendar.DATE)) {
                    //here return like "Today at 12:00"
                    resUtil.getStringRes(R.string.txt_today) + " " + DateFormat.format("HH:mm", neededTime)

                } else if (nowTime.get(Calendar.DATE) - neededTime.get(Calendar.DATE) == 1) {
                    //here return like "Yesterday at 12:00"
                    resUtil.getStringRes(R.string.txt_ysterday) + " " + DateFormat.format("HH:mm", neededTime)
    ...
    fun getApplicantsStr(amount: Int) = "" + amount + " " + resUtil.getStringRes(R.string.txt_aplicants)
